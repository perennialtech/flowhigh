...
class FLowHighTrainer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.dataloader = get_dataloader(
            self.train_ds, batch_size=batch_size, shuffle=True, drop_last=drop_last
        )
        self.valid_dataloader = get_dataloader(
            self.valid_ds, batch_size=1, shuffle=True, drop_last=drop_last
        )
        ...
        self.cfm_wrapper, self.optim, self.scheduler, self.dataloader = (
    ...
    def train_step(self):
        ...
        for grad_accum_step in range(self.grad_accum_every):
            ...
            context = (
                partial(self.accelerator.no_sync, self.cfm_wrapper)
                if not is_last
                else nullcontext
            )
        ...
        if exists(self.max_grad_norm):
            self.accelerator.clip_grad_norm_(
                self.cfm_wrapper.parameters(), self.max_grad_norm
            )
        ...
        if not steps % self.log_every:
            ...
            self.print(
                f"Epoch {current_epoch}, Step {steps}: loss: {logs['loss']:0.3f}"
            )
    ...
    def train(self, mid_ckpt=None, log_fn=noop):
        ...
        if mid_ckpt is not None:
            if os.path.exists(mid_ckpt):
                ...
                print(
                    f"No checkpoint found at {mid_ckpt}, starting training from scratch"
                )
        else:

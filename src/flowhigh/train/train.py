...
if __name__ == "__main__":
    ...
    dataset = AudioDataset(
        folder=hparams.data.data_path, downsampling=hparams.data.downsampling_method
    )
    ...
    sampling_rates = list(
        range(hparams.data.downsample_min, hparams.data.downsample_max + 1000, 1000)
    )

    print("Initializing Mel vocoder...")
    ...
    cfm_wrapper = ConditionalFlowMatcherWrapper(
        flowhigh=model, cfm_method=hparams.model.cfm_path, sigma=hparams.model.sigma
    )
